...
class MenuListJUnitTests {
	...
	void testAddToListItemID() {
		MenuList ml = new MenuList();
		Item mi = new Item("FOOD", "Croissant", "French Nature Pastry.", 1, 3.5);
		
		Throwable exception = assertThrows(IllegalStateException.class, () -> ml.addToList(mi));
		assertEquals("ItemID is too short.", exception.getMessage(), "Setting an ItemID too short should not work.");
		
		Item mi2 = new Item("FOAD1", "Croissant", "French Nature Pastry.", 1, 3.5);
		Throwable exceptionB = assertThrows(IllegalStateException.class, () -> ml.addToList(mi2));
		assertEquals("ItemID category identifier incorrect.", exceptionB.getMessage(), "Setting a wrong ItemID category identifier should not work.");
		
		Item mi3 = new Item("FOODA", "Croissant", "French Nature Pastry.", 1, 3.5);
		Throwable exceptionC = assertThrows(NumberFormatException.class, () -> ml.addToList(mi3));
		assertEquals("ItemID doesn't have a number identifier.", exceptionC.getMessage(), "Setting no ItemID number identifier should not work.");
